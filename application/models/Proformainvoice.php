...
Class Proformainvoice extends CI_Model {
    ...
    public function get_proforma_invoices($uid, $limit = 1000) {
        ...
        if (!empty($fy_year)) {
            ...
            $this->db->where('proforma_invoice_total.date >=', $fy_from);
            $this->db->where('proforma_invoice_total.date <=', $fy_to);
        }

        $this->db->select('* , SUM(invocie_pay_amount) as total_balance_amount');
        $this->db->from('proforma_invoice');
      //  $this->db->where('proforma_invoice.uid', $uid);
       // $this->db->where('proforma_invoice_total.uid', $uid);
    ...
    public function add_invoice($data_invoice) {
        ...
        private $proforma_columns_ensured = false;

        private function ensureProformaTextColumns() {
            if ($this->proforma_columns_ensured) {
                return;
            }

            $this->proforma_columns_ensured = true;

            try {
                $required_columns = array(
                    'proforma_terms_and_conditions' => array('type' => 'TEXT', 'null' => TRUE),
                    'proforma_payment_terms' => array('type' => 'TEXT', 'null' => TRUE),
                    'proforma_process_schedule' => array('type' => 'TEXT', 'null' => TRUE),
                    'proforma_taxes' => array('type' => 'TEXT', 'null' => TRUE),
                    'proforma_exclusions' => array('type' => 'TEXT', 'null' => TRUE)
                );

                $existing_columns = $this->db->list_fields('proforma_invoice_total');
                $this->load->dbforge();

                foreach ($required_columns as $column_name => $column_definition) {
                    if (!in_array($column_name, $existing_columns, true)) {
                        $this->dbforge->add_column('proforma_invoice_total', array($column_name => $column_definition));
                        $existing_columns[] = $column_name;
                    }
                }

                unset($this->table_columns_cache['proforma_invoice_total']);
            } catch (Throwable $e) {
                // If schema alteration is not allowed, save flow still continues with available columns.
            }
        }

        private function filterDataByExistingColumns($table, $data) {
            if (!is_array($data)) {
                return array();
            }

            if ($table === 'proforma_invoice_total') {
                $this->ensureProformaTextColumns();
            }

            if (!isset($this->table_columns_cache[$table])) {
                $this->table_columns_cache[$table] = $this->db->list_fields($table);
            }

            $valid_columns = array_flip($this->table_columns_cache[$table]);
            return array_intersect_key($data, $valid_columns);
        }

    private function normalizeProformaTextFields($row) {
        if (!is_array($row)) {
            return array();
        }

        $field_map = array(
            'proforma_terms_and_conditions' => 'terms_and_conditions',
            'proforma_payment_terms' => 'payment_terms',
            'proforma_process_schedule' => 'process_schedule',
            'proforma_taxes' => 'taxes',
            'proforma_exclusions' => 'exclusions'
        );

        foreach ($field_map as $proforma_key => $generic_key) {
            if (empty($row[$proforma_key]) && isset($row[$generic_key])) {
                $row[$proforma_key] = $row[$generic_key];
            }
        }
    ...
    public function add_proforma_invoice_total($data_invoice_total) {
            $filtered_data = $this->filterDataByExistingColumns('proforma_invoice_total', $data_invoice_total);
            return $this->db->insert('proforma_invoice_total', $filtered_data);
    }
    ...
    public function delete_proforma_invoice_by_invoice_number($invoice_number, $uid) {
        
        //$this->db->where('uid', $uid);
        $this->db->where('invoice_number', $invoice_number);
        $this->db->delete('proforma_invoice');
        if ($this->db->affected_rows() >= 1) {
            $this->db->where('number_fk', $invoice_number);
            $this->db->delete('proforma_invoice_total');
...
//            $this->db->delete('proforma_payment_gst');
    ...
    public function get_last_proforma_invoice_number($uid) {
...
//        return $result;
    ...
    public function update_proforma_invoice_total($invoice_number, $quote_number, $uid) {
        $this->db->where('number_fk', $quote_number);
        //$this->db->where('uid', $uid);
        $filtered_data = $this->filterDataByExistingColumns('proforma_invoice_total', $invoice_number);
        $this->db->update('proforma_invoice_total', $filtered_data);
        if ($this->db->affected_rows() == '1') {
            return TRUE;
        } else {
            return FALSE;
    ...
    public function get_proforma_invoice_data($invoice_number, $uid) {
        $this->db->select('pi.*, inventory.item_name');
        $this->db->from('proforma_invoice pi');
        $this->db->where('pi.invoice_number', $invoice_number);
        $this->db->join('inventory', 'inventory.code=pi.product_name', 'Left Join');
        //$this->db->where('pi.uid', $uid);
        $query = $this->db->get();
        return $query->result();
    }
    ...
    public function get_proforma_invoice_data_group_by($invoice_number, $uid) {
      //  print_r($invoice_number);die();
       ...
       // $this->db->where('proforma_invoice_total.uid', $uid);
        ...
        return $this->normalizeProformaTextFields($query->row_array());
    }

    public function edit_invoice_payment($data_payment, $id) {
     
        $this->db->where('id', $id);
        $this->db->update('proforma_invoice_total', $data_payment);
        if ($this->db->affected_rows() == '1') {
            return TRUE;
        } else {
            return FALSE;
...
//    }
    ...
    public function get_proforma_invoice_data_by_status($status, $uid) {
        $this->db->select('* , SUM(invocie_pay_amount) as total_balance_amount');
        $this->db->from('proforma_invoice');
        $this->db->join('customer', 'customer.customer_id=proforma_invoice.customer_id', 'Left Join');
        $this->db->join('proforma_invoice_total', 'proforma_invoice_total.number_fk=proforma_invoice.invoice_number', 'Right Join');
...
//    }
    ...
    public function get_proforma_invoice_count($uid) {
        $this->db->select('*');
        $this->db->from('proforma_invoice_total');
        //$this->db->where('uid', $uid);
        $query = $this->db->get();
    ...
    public function get_proforma_invoice_draft_count($draft_status, $uid) {
      ...
      public function get_proforma_invoice_send_count($sent_status, $uid) {
   ...
   //     $this->db->where('invoice.uid', $uid);
    ...
    public function edit_invoice_status($data_customer, $invoice_number, $uid) {
        $this->db->where('number_fk', $invoice_number);
        //$this->db->where('uid', $uid);
        $this->db->update('proforma_invoice_total', $data_customer);
        if ($this->db->affected_rows() == '1') {
            return TRUE;
        } else {
            return FALSE;
    ...
    public function get_pay_gst_invoice_amount($invoice_number, $uid) {
       ...
       // echo $invoice_number;
        ...
        $this->db->from('proforma_payment_gst');
        //$this->db->where('uid', $uid);
        $this->db->where('invoice_number_fk', $invoice_number);
        $query = $this->db->get();
        return $query->result();
    ...
    public function get_proforma_invoice_payment_history_data($invoice_number, $uid) {
        $this->db->select('*');
        $this->db->from('proforma_payment_gst');
        $this->db->where('invoice_number_fk', $invoice_number);
        //$this->db->where('uid', $uid);
   ...
   public function pay_gst_proforma_amount($invoice_payment_gst) {
        ...
        return $this->db->insert('proforma_payment_gst', $invoice_payment_gst);
    }
      ...
      public function print_proforma_voucher($invocie_pay_id, $uid){
       ...
       $this->db->join('customer', 'customer.customer_id=proforma_payment_gst.customer_id_fk', 'Left Join');
 ...
 public function get_last_proforma_number($uid) {
       ...
       $result = $query->row();
        ...
        $keywords = explode('/', $result->number_fk);
        $var = $keywords[1];
        $var1 = ltrim($var, '0');
        //echo $var1; die();
        return $var1;
    ...
    public function get_pf_number_from_pf_total($id, $uid) {
        $this->db->select('number_fk');
        $this->db->from('proforma_invoice_total');
        $this->db->where('id', $id);
        //$this->db->where('uid', $uid);
    ...
    public function get_convert_pf_data($number, $uid) {
        $this->db->select('*, proforma_invoice.gst as gst');
      $this->db->from('proforma_invoice');
