...
<body class="hold-transition skin-blue sidebar-mini">
    ...
    <script>
        ...
        function updateOALetterPreview() {
            ...
            var number = $('#number').val() || "<?php echo isset($oc['number_fk']) ? $oc['number_fk'] : 'OC-001/26-27'; ?>";
            var dateInput = $('#date').val();
            var dateVal = dateInput ? new Date(dateInput).toLocaleDateString('en-GB') : new Date().toLocaleDateString('en-GB');
            var poRef = $('#po_reference').val() || "<?php echo isset($oc['po_reference']) ? addslashes($oc['po_reference']) : '4520232398'; ?>";
            var poDateInput = $('#po_date').val();
            var poDateVal = poDateInput ? new Date(poDateInput).toLocaleDateString('en-GB') : '31.12.2025';
            var subject = $('#subject').val() || "<?php echo isset($oc['subject']) ? addslashes($oc['subject']) : 'DOSING SYSTEM for WTP Plant (Hindalco) (W-26004)'; ?>";
            ...
            var subTotalVal = $('#sub_total').val() || "<?php echo isset($oc['sub_total']) ? number_format($oc['sub_total'], 2) : '31,75,000'; ?>";
            var taxAmountVal = parseFloat($('#tax_amount').val()) || <?php echo isset($oc['tax_amount']) ? (float)$oc['tax_amount'] : 0; ?>;
