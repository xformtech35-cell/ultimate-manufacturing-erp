...
if(!isset($purchase_order_item_report1) || empty($purchase_order_item_report1)) {
    ...
    $sheet->setCellValue('A1', 'No sale order item data found for the selected criteria');
    $sheet->getStyle('A1')->getFont()->setBold(true)->setSize(14);
    $sheet->getStyle('A1')->getFont()->getColor()->setARGB(Color::COLOR_RED);
    
    $file_name = 'Sale_Order_Item_Report_No_Data.xlsx';
...
// Note: The variable name is purchase_order_item_report1 but this appears to be for Sale Order Items
// Based on the filename and headers, I'll keep the display name as Sale Order Item Report

// Rename the sheet
$sheet->setTitle('Sale Order Item Report');

// Main Header
$sheet->mergeCells('A1:D1');
$sheet->setCellValue('A1', 'SALE ORDER ITEM REPORT');
...
$sheet->mergeCells('A' . $itemSummaryRow . ':D' . $itemSummaryRow);
$sheet->setCellValue('A' . $itemSummaryRow, 'ITEM-WISE SUMMARY');
...
$sheet->setCellValue('C' . $itemSummaryRow, 'Amount (₹)');
...
$sheet->mergeCells('A' . $topItemsRow . ':D' . $topItemsRow);
$sheet->setCellValue('A' . $topItemsRow, 'TOP ITEMS BY VALUE');
...
$sheet->setCellValue('D' . $topItemsRow, 'Amount (₹)');
$sheet->getStyle('A' . $topItemsRow . ':D' . $topItemsRow)->applyFromArray(['font' => ['bold' => true]]);
$topItemsRow++;
...
foreach ($top_items as $item_name => $data) {
    ...
    $sheet->getStyle('C' . $topItemsRow . ':D' . $topItemsRow)->applyFromArray($rightAlignStyle);
...
// Date Range Summary
$dateRangeRow = $topItemsRow + 2;

$sheet->mergeCells('A' . $dateRangeRow . ':D' . $dateRangeRow);
...
$sheet->setCellValue('C' . $dateRangeRow, '');
$sheet->setCellValue('D' . $dateRangeRow, '');
...
foreach ($summary_data as $data) {
    ...
    if(is_numeric($data[1])) {
        ...
        if(strpos($data[0], 'Revenue') !== false || strpos($data[0], 'Amount') !== false) {
...
$sheet->setCellValue('A' . $footerRow, 'This is a system-generated sale order item report. For any discrepancies, please contact the sales department.');
...
$file_name = 'Sale_Order_Item_Report_' . $customer_display . '_' . $from_date_display . '_to_' . $to_date_display . '.xlsx';
