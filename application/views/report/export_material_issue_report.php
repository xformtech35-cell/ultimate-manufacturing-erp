...
$columnDefs = [
    ...
    ['title' => 'Issue Slip No.', 'key' => 'issue_no', 'type' => 'text'],
    ['title' => 'Issue Date', 'key' => 'issue_date', 'type' => 'date'],
];
...
array_push($columnDefs,
    ...
    ['title' => 'Quantity', 'key' => 'qty', 'type' => 'number'],
    ['title' => 'Issued Qty', 'key' => 'issuedQty', 'type' => 'number'],
...
$sheet->setTitle('Material Issue Report');

// Main Header
$sheet->mergeCells("A1:{$maxCol}1");
$sheet->setCellValue('A1', 'MATERIAL ISSUE REPORT');
...
foreach ((array) $result as $row) {
    ...
    $issuedQty = isset($row->issued_qty) ? (float) $row->issued_qty : 0;
    $costPrice = isset($row->cost_price) ? (float) $row->cost_price : 0;
    $lineTotal = isset($row->total_cost) ? (float) $row->total_cost : ($issuedQty * $costPrice);
...
if (!empty($result)) {
    foreach ($columnDefs as $idx => $def) {
        ...
        if ($def['key'] === 'item_name') {
            ...
            $sheet->setCellValue($colLetter . $rowNo, $totalIssuedQty);
            $sheet->getStyle($colLetter . $rowNo)->getNumberFormat()->setFormatCode('#,##0.00');
...
$fileName = 'Material_Issue_Report_' . date('d-m-Y_H-i-s') . '.xlsx';
