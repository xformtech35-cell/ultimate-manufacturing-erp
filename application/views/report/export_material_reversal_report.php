...
$columnDefs = [
    ...
    ['title' => 'Slip / Reversal No.', 'key' => 'issue_no', 'type' => 'text'],
    ['title' => 'Date', 'key' => 'issue_date', 'type' => 'date'],
];
...
array_push($columnDefs,
    ...
    ['title' => 'Required Qty', 'key' => 'qty', 'type' => 'number'],
    ['title' => 'Reversed Qty', 'key' => 'issuedQty', 'type' => 'number'],
...
$sheet->setTitle('Material Reversal Report');

// Main Header
$sheet->mergeCells("A1:{$maxCol}1");
$sheet->setCellValue('A1', 'MATERIAL REVERSAL REPORT');
...
foreach ((array) $result as $row) {
    ...
    $rawIssuedQty = isset($row->issued_qty) ? (float) $row->issued_qty : 0;
    $issuedQty = abs($rawIssuedQty);
    $costPrice = isset($row->cost_price) ? (float) $row->cost_price : 0;
    $rawLineTotal = isset($row->total_cost) ? (float) $row->total_cost : ($rawIssuedQty * $costPrice);
    $lineTotal = abs($rawLineTotal);
    
    $totalReversedQty += $issuedQty;
    $totalCost += $lineTotal;

    $rowValues = [
        'sr' => $sr,
        'issue_no' => isset($row->issue_no) ? $row->issue_no : '',
        'issue_date' => !empty($row->issue_date) ? date('d-m-Y', strtotime($row->issue_date)) : '',
...
if (!empty($result)) {
    foreach ($columnDefs as $idx => $def) {
        ...
        } else if ($def['key'] === 'issuedQty') {
            $sheet->setCellValue($colLetter . $rowNo, $totalReversedQty);
            $sheet->getStyle($colLetter . $rowNo)->getNumberFormat()->setFormatCode('#,##0.00');
...
$fileName = 'Material_Reversal_Report_' . date('d-m-Y_H-i-s') . '.xlsx';
