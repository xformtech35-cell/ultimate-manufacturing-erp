...
<div class="content-wrapper">
    <section class="content-header">
        <h1>
            <i class="fa fa-list-alt"></i> Material Allocation Report
        </h1>
        <ol class="breadcrumb">
            <li><a href="<?php echo base_url() . 'Home/index/' ?>"><i class="fa fa-dashboard"></i> Home</a></li>
            <li><a href="#">Report</a></li>
            <li class="active">Material Allocation Report</li>
        </ol>
    </section>
            ...
            <div class="col-md-12">
                <div class="box box-primary">
                    <div class="box-header with-border">
                        <h3 class="box-title"><i class="fa fa-filter"></i> Filter Material Allocation Report</h3>
                    </div>
                    <form class="form-horizontal form_overlay" method="post" action="<?php echo base_url(); ?>ReportController/material_allocation_report">
                        <div class="box-body">
                ...
                <div class="box box-info">
                    <div class="box-header with-border">
                        <h3 class="box-title"><i class="fa fa-table"></i> Material Allocation Details</h3>
                        <?php if ($is_filtered) { ?>
                            <div class="box-tools pull-right">
                                <a href="<?php echo base_url(); ?>ReportController/get_material_allocation_report_xlsx" class="btn btn-sm btn-success">
                                    <i class="fa fa-file-excel-o"></i> Export to Excel
                                </a>
                                <a href="<?php echo base_url(); ?>ReportController/get_material_allocation_report_pdf" class="btn btn-sm btn-danger">
                    ...
                    <div class="box-body">
                            ...
                            <table id="example3" class="table table-bordered table-striped table-hover" style="width:100%;">
                                ...
                                <tbody>
                                    ...
                                    foreach ($result as $row) {
                                        ...
                                        $pending_qty = isset($row->pending_quantity) ? (float) $row->pending_quantity : 0;
                                        $cost_price = isset($row->cost_price) ? (float) $row->cost_price : 0;
                                        $line_total = isset($row->total_cost) ? (float) $row->total_cost : ($alloc_qty * $cost_price);
                                        ...
                                        if (in_array($st, ['allocated', 'issued', 'approved', 'completed'])) {
                                            $status_badge = 'label-success';
                                        } else if (in_array($st, ['draft', 'pending', 'partial'])) {
                                    ...
                                    ?>
                                        <tr>
                                            ...
                                            <td class="text-center"><?php echo !empty($row->allocated_date) ? date('d-m-Y', strtotime($row->allocated_date)) : ''; ?></td>
                                            <?php if ($show_project_cols): ?>
                                            ...
                                            <td class="text-right font-weight-bold"><?php echo number_format($alloc_qty, 2); ?></td>
                                            ...
                                            <td class="text-right"><?php echo number_format($pending_qty, 2); ?></td>
                                            <td class="text-center">
                                ...
                                <?php if (!empty($result)) { ?>
                                    <tfoot>
                                        <tr style="font-weight:bold;background:#e8f442;">
                                            <td colspan="<?php echo $show_project_cols ? '9' : '7'; ?>" class="text-right">Total:</td>
                                            <td class="text-right"><?php echo number_format($total_allocated, 2); ?></td>
                                            <td></td>
                                            <td class="text-right"><?php echo number_format($total_cost, 2); ?></td>
                                            <td class="text-right"><?php echo number_format($total_pending, 2); ?></td>
                                            <td colspan="2"></td>
                                        </tr>
                                    </tfoot>
                                <?php } ?>
                        ...
                        <?php if ($is_filtered && empty($result)) { ?>
                            <div class="alert alert-info text-center" style="margin-top:15px;">
                                <strong>No material allocation records found</strong> for the selected date range.
                            </div>
                        <?php } ?>
                    </div>
                </div>
...
$(document).ready(function() {
    ...
    $('#example3').DataTable({
        ...
        "pageLength": 25,
        "language": {
            "search": "Search Material Allocation:"
        }
    });
});
</script>
