...
if(!isset($result) || empty($result)) {
    ...
    $sheet->setCellValue('A1', 'No sale report data found for the selected date range');
    $sheet->getStyle('A1')->getFont()->setBold(true)->setSize(14);
    $sheet->getStyle('A1')->getFont()->getColor()->setARGB(Color::COLOR_RED);
    
    $file_name = 'Sale_Report_No_Data.xlsx';
...
$sheet->setCellValue('A1', 'SALE REPORT');
...
if($rowCou > 1) {
    ...
    $sheet->setCellValue('B' . $totalRow, 'TOTAL SALE AMOUNT:');
...
$sheet->setCellValue('C' . $summaryRow, 'Average Sale: ₹ ' . number_format(($rowCou > 1 ? $total_amount / ($rowCou - 1) : 0), 2));
...
$sheet->setCellValue('A' . $footerRow, 'This is a system-generated sale report. For any discrepancies, please contact the sales department.');
...
$file_name = 'Sale_Report_' . $from_date_display . '_to_' . $to_date_display . '.xlsx';
